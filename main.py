...
def forwardFT( image ):
  ...
  transformedImage = np.empty(image.shape, dtype = "complex")

  for row in range(rows):
    transformedImage[row, :] = ft1D(image[row, :])

  for col in range(cols):
    transformedImage[:, col] = ft1D(transformedImage[:, col])
...
def inverseFT( image ):
  ...
  for row in range(rows):
    transformedImage[row, :] = ift1D(image[row, :])
